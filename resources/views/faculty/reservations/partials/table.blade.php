                        ...
                        <td class="px-4 py-2 text-center">
                            ...
                            @if(auth()->user()->role === 'admin')
                                @if($reservation->status === 'pending')
                                    ...
                                    <form action="{{ route('faculty.reservations.approve', $reservation->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-green-600 hover:underline ml-2">Approve</button>
                                    </form>
                                    <form action="{{ route('faculty.reservations.reject', $reservation->id) }}" method="POST" class="inline">
                                        @csrf
