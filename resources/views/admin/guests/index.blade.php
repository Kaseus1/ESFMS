...
<div class="container mx-auto px-4 py-8">
    ...
    @if(session('success') || session('error'))
        <div x-data="{ show: true }" 
             ...
             class="mb-6 rounded-lg border p-4 shadow-sm flex items-start justify-between {{ session('success') ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800' }}">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-medium">
                        {{ session('success') ? 'Success' : 'Action Failed' }}
                    </h3>
            ...
            <button @click="show = false" class="ml-4 inline-flex flex-shrink-0 rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2 {{ session('success') ? 'text-green-500 hover:bg-green-100' : 'text-red-500 hover:bg-red-100' }}">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-[#333C4D] border-opacity-10 bg-[#F8F9FA]">
            <h2 class="text-base font-bold text-[#172030]">Guest Accounts</h2>
        </div>
                        ...
                        @foreach($guests as $guest)
                            <tr class="hover:bg-[#F8F9FA] transition">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-[#333C4D] opacity-75">{{ $guest->contact_number ?? 'N/A' }}</div>
                                </td>

                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-[#172030]">{{ $guest->organization ?? 'N/A' }}</div>
                                    <div class="text-xs text-[#333C4D] opacity-75 truncate max-w-[200px]" title="{{ $guest->purpose }}">
                                        {{ Str::limit($guest->purpose ?? 'No purpose specified', 40) }}
                                    </div>
                                </td>
