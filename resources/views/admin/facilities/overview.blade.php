...
<div x-data="facilityDashboard()" x-init="init()" class="mb-6">
    ...
    <div x-show="loading" class="fixed inset-0 flex items-center justify-center bg-white bg-opacity-50 z-50">
        <div class="loader ease-linear rounded-full border-4 border-t-4 border-[#333C4D] border-opacity-20 h-12 w-12"></div>
    </div>
    ...
    <div class="mb-4 flex flex-wrap gap-2">
        <template x-for="filter in filterOptions" :key="filter.value">
            <a
                ...
                :class="status === filter.value ? 'bg-[#002366] text-white' : 'bg-[#333C4D] border-opacity-20 text-[#333C4D] border border-[#333C4D]'"
            >
    ...
    <div class="mb-4">
        <input
            ...
            class="w-full px-4 py-2 border border-[#333C4D] border-opacity-20 rounded shadow-sm focus:outline-none focus:ring focus:ring-[#002366] focus:border-[#002366]"
        >
    </div>

    {{-- Table --}}
    <div class="bg-white shadow rounded-lg p-6 border-t-4 border-[#172030]">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-[#172030]">Facility Overview</h2>
            <a href="{{ route('facilities.create') }}"
               class="inline-flex items-center px-4 py-2 bg-[#002366] text-white text-sm font-semibold rounded-lg shadow hover:bg-[#00285C] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#002366] transition">
                + Add Facility
            </a>
        </div>

        <div x-show="!hasFacilities && !loading" class="text-center py-6 text-[#333C4D] font-medium">
            <p x-transition.opacity.duration.500ms>No facilities match your search or filter.</p>
        </div>

        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-[#333C4D] border-opacity-20" x-show="hasFacilities" x-transition.opacity.duration.500ms>
                <thead class="bg-[#F8F9FA]">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#333C4D] uppercase tracking-wider">Next Reservation</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#333C4D] uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-[#333C4D] border-opacity-20">
                    <template x-for="row in tableRows" :key="row.id">
                        <tr class="hover:bg-[#F8F9FA]" x-html="row.html"></tr>
                    </template>
                </tbody>
            </table>
        </div>
...
function facilityDashboard() {
    return {
        ...
        paginationHtml: '',
        cards: [
            { title: 'Total Facilities', count: {{ $totalFacilities }}, colorClass: 'border-[#002366]', textClass: 'text-[#002366]', filter: '' },
